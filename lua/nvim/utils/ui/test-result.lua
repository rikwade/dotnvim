...
function M:new(test_result_sub)
    ...
    test_result_sub.add_listener(
        TestEventType.COMPLETE, function(...)
            M:_on_test_complete(...)
        end)
    return self
end
...
function M:show()
    ...
    Shortcut:mode('n'):buffer(popup.bufnr):options():noremap():next():keymaps(
        { { '<cr>', 'zA' }, { 'o', 'zA' } })
    ...
    for _, result in ipairs(self.results) do
        for _, line in ipairs(result) do table.insert(lines, line) end
    end
